...
// マクロ定義
//*****************************************************************************
#define	TEXTURE_EXPLOSION		"data/TEXTURE/explosion02.png"	// 読み込むテクスチャファイル名
#define	VALUE_MOVE_EXPLOSION	(2.0f)							// 移動速度
#define TEXTURE_X				(10)
#define	MAX_EXPLOSION			(4)							// ビルボード最大数
...
HRESULT InitExplosion(void)
{
	...
	for(int nCntExplosion = 0; nCntExplosion < MAX_EXPLOSION; nCntExplosion++)
	{
		...
		g_aExplosion[nCntExplosion].fSizeX = SCREEN_WIDTH;
		g_aExplosion[nCntExplosion].fSizeY = SCREEN_HEIGHT;
	...
	for(int nCntExplosion = 0; nCntExplosion < MAX_EXPLOSION; nCntExplosion++)
	{
		if(g_aExplosion[nCntExplosion].bUse)
		{
			...
			if((g_aExplosion[nCntExplosion].nCounter % 4) == 0)
			{
				...
				if(g_aExplosion[nCntExplosion].nPattern >= 16)
				{
					g_aExplosion[nCntExplosion].bUse = false;
				}
				else
			...
			if(g_aExplosion[nCntExplosion].nPattern >= 1)
			{
				// 頂点カラーの設定
...
void SetTextureExplosion(int nIdxExplosion, int nPattern)
{
	{//頂点バッファの中身を埋める
		...
		pVtx += (nIdxExplosion * 4);
		float sizeX = 1.0f / TEXTURE_X;
			;
		// 頂点座標の設定
		pVtx[0].tex = D3DXVECTOR2(nPattern * sizeX, 0.0f);
		pVtx[1].tex = D3DXVECTOR2((nPattern + 1) * sizeX, 0.0f);
		pVtx[2].tex = D3DXVECTOR2(nPattern * sizeX, 1.0f);
		pVtx[3].tex = D3DXVECTOR2((nPattern + 1) * sizeX, 1.0f);
