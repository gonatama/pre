...
#define	INTERVAL_CAMERA_R	(20.0f)					// モデルの視線の先までの距離
...
void UpdateCamera(void)
{
	...
	g_posCameraPDest.x = posPlayer.x - sinf(rotCamera.y) * g_fLengthIntervalCamera - sin(rotPlayer.y) * fLength;
	g_posCameraPDest.y = posPlayer.y + CHASE_HEIGHT_P;
	g_posCameraPDest.z = posPlayer.z - cosf(rotCamera.y) * g_fLengthIntervalCamera - cos(rotPlayer.y) * fLength;
